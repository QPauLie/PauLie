...
class RecordingMorphFactory(MorphFactory):
    ...
    def _get_pq(self, lighting:PauliString
    ...
    def _append_to_two_center(self, lighting:PauliString) -> None:
        ...
        if len(lits) == 1:
            if center in lits:
                self._append(lighting, center)
                recording_graph(self.record, lighting=lighting, lits=self._get_lits(lighting),
                appending=center, title=f"Step I: {lighting}")
                return
            else:
        ...
        if pq is not None:
            ...
            for lit in lits:
                if lit != p:
                    ...
                    self._replace(lit, v)
                    replacing.append(lit)
            recording_graph(self.record, collection=self._get_vertices(),lighting=lighting,
            lits=self._get_lits(lighting), title=f"Step II: {lighting}")
            ...
            recording_graph(self.record, collection=self._get_vertices(), lighting=lighting,
            lits=self._get_lits(lighting), appending=p, title=f"Step II: {lighting}")
            raise AppendedException
        self._set_lighting(lighting)
        return self
    ...
    def _reduce_long_leg_more_than_one_lits(self) -> Self:
        ...
        while True:
            ...
            if len(lits) == 0:
                ...
                recording_graph(self.record, lighting=lighting, lits=self._get_lits(lighting),
                appending=self._get_center(), title=f"Step IV: {lighting}")
                raise AppendedException()
            ...
            if len(lits) == 1:
                ...
                if long_leg[0] != lits[0]:
                    ...
                    if lit_indexes[0] < len(long_leg) - 1:
                        ...
                        recording_graph(self.record, collection=self._get_vertices(),
                        lighting=lighting,
                        lits=self._get_lits(lighting), title=f"Step IV: {lighting}")
                    break
            lit_indexes = self._get_lit_indexes(long_leg, lits)
            first = lit_indexes[0]
    ...
    def _append_long_leg_only_last_lit(self) -> Self:
        ...
        if len(lits) == 1:
            ...
            self._remove(last_v)
            recording_graph(self.record, collection=self._get_vertices(), lighting=lighting,
            lits=self._get_lits(lighting), title=f"Step VI: {lighting}")
            recording_graph(self.record, lighting=lighting, lits=self._get_lits(lighting),
            appending=center, title=f"Step VI: {lighting}")
            self._append(lighting, center)
            recording_graph(self.record, collection=self._get_vertices(), lighting=lighting,
            lits=self._get_lits(lighting), title=f"Step VI: {lighting}")
            recording_graph(self.record, lighting=last_v, lits=self._get_lits(lighting),
            replacing_vertices=[g], title=f"Step VI: {lighting}")
            self._replace(g, new_g)
            recording_graph(self.record, collection=self._get_vertices(), lighting=last_v,
            lits=self._get_lits(lighting), title=f"Step VI: {lighting}")
            long_leg = self._get_long_leg()
            if len(long_leg) > 4:
                removing = []
                ...
                self._remove(long_leg[4])
            recording_graph(self.record, collection=self._get_vertices(), lighting=last_v,
            lits=self._get_lits(lighting), appending=lighting, title=f"Step VI: {lighting}")
