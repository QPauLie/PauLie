...
def average_otoc(generators: PauliStringCollection, v: PauliString, w: PauliString) -> float:
    r"""
    Finds the Haar averaged out-of-time-order correlator :math:`F` between two Pauli strings :math:`V` and :math:`W`.
    ...
    where :math:`U` is sampled according to the Haar measure on the Lie group corresponding to
    the DLA generated by `generators` and :math:`d` is the dimension of the Hilbert space.
    ...
    Returns:
        float: Haar averaged out-of-time-order correlator :math:`F` between two Pauli strings :math:`V` and :math:`W`.
    """
