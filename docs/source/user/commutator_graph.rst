...
Fixed unitary, uniform Paulis, and Pauli instability
-----------------------------------------------------

The :func:`~paulie.application.otoc.average_otoc` function above averages the OTOC over **Haar-random** evolution :math:`U` drawn from the Lie group generated by a Pauli-string DLA (see the commutator-graph discussion and :mod:`paulie.application.otoc`). That is a different ensemble from the following **fixed-unitary** construction.

For a chosen unitary :math:`U` on :math:`n` qubits (:math:`d = 2^n`) and independent **uniform** Pauli strings :math:`P_1, P_2` on :math:`\{I,X,Y,Z\}^{\otimes n}`, define

.. math::

    \mathrm{OTOC}(U, P_1, P_2) = \frac{1}{d}\,\mathrm{Tr}\!\left(
    U^\dagger P_1 U P_2 U^\dagger P_1 U P_2\right).

The function :func:`~paulie.application.pauli_instability.otoc_fixed_unitary` evaluates this quantity for explicit :math:`P_1,P_2`. The **mean absolute OTOC** over the uniform product measure on :math:`(P_1,P_2)` is computed by :func:`~paulie.application.pauli_instability.mean_abs_otoc_uniform`, and **Pauli instability** (Definition 1) is

.. math::

    I(U) = -\log \mathbb{E}_{P_1,P_2}\bigl[\lvert\mathrm{OTOC}(U,P_1,P_2)\rvert\bigr],

via :func:`~paulie.application.pauli_instability.pauli_instability`.

**Exact vs Monte Carlo.** Summing over all :math:`16^n` Pauli pairs is implemented as ``method="exact"`` but is **only allowed for at most four qubits** to avoid accidental huge loops. For more qubits, use ``method="monte_carlo"`` with ``num_samples`` as needed.

**Randomness.** Monte Carlo draws use :class:`numpy.random.Generator`. Pass an integer ``seed`` to :func:`~paulie.application.pauli_instability.mean_abs_otoc_uniform` or :func:`~paulie.application.pauli_instability.pauli_instability` for reproducible estimates; with ``seed=None`` each call uses a new default generator.
