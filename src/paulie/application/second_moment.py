"""
    Module for computing the second-order twirl of an operator over a group generated by system
    generators, based on the theory in arXiv:2502.16404.
"""
...
def second_moment(
    ...
    """
    Computes the second-order twirl of an operator :math:`M` by projecting it onto the subspace of
    quadratic symmetries of a DLA.

    Args:
        operator_m (PauliStringLinear): An operator :math:`M` expressed as a linear combination of
            Pauli strings.
    ...
    simplified = twirl_result.simplify()
    if isinstance(simplified, PauliStringLinear):
        return simplified
    raise TypeError("Simplify did not return a PauliStringLinear")
