...
Pauli instability and ``average_otoc``
--------------------------------------

The preceding ``average_otoc`` example forms an expectation by drawing **Haar-random** dynamics :math:`U`
from the Lie group generated by your DLA. **Pauli instability** (Definition~1 in :cite:t:`Garcia_2024_01663`)
instead fixes a **chosen** unitary :math:`U` and forms an expectation over **independent uniform** Pauli
strings :math:`P_1,P_2` on :math:`\{I,X,Y,Z\}^{\otimes n}` (and the OTOC built from those). Both are
averages over random inputs, but the randomness runs over **different** degrees of freedom: :math:`U` vs
:math:`(P_1,P_2)`.

There, ``pauli_instability`` quantifies **magic** (non-Clifford resource) via OTOCs. Corollary~1 of that work highlights when such a quantity can be **measured efficiently** (Pauli sample complexity polynomial in :math:`n`) versus when accurate estimation requires resources exponential in system size — a useful lens when you interpret Monte Carlo estimates from ``mean_abs_otoc_uniform`` or ``pauli_instability``.

Definitions (Definition~1), conventions, and implementation details for those helpers live in the API docs for :mod:`paulie.application.otoc`. Below we only **call** ``pauli_instability`` on prototypical gates.

.. code-block:: python

    import numpy as np
    from paulie import pauli_instability

    # Single-qubit Hadamard is Clifford → Pauli instability vanishes (faithfulness).
    h = np.array([[1, 1], [1, -1]], dtype=np.complex128) / np.sqrt(2)
    print("Pauli instability of H:", round(pauli_instability(h, method="exact"), 6))

    # Single-qubit T gate: I(T) = log(4/3) (scaling with T gates, k=1 in the paper).
    t = np.diag([1.0, np.exp(1j * np.pi / 4)]).astype(np.complex128)
    print("Pauli instability of T:", round(pauli_instability(t, method="exact"), 6))
    print("log(4/3)             :", round(np.log(4.0 / 3.0), 6))

Typical output:

.. code-block:: bash

    Pauli instability of H: 0.0
    Pauli instability of T: 0.287682
    log(4/3)             : 0.287682
