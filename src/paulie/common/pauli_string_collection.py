...
class PauliStringCollection:
    ...
    def get_algebra_basis(self) -> list[np.ndarray]:
        """
        Get the defining-representation matrix basis of the dynamical Lie
        algebra generated by the set of Pauli strings, partitioned by direct
        summand.

        Where :meth:`get_algebra` returns the name of the algebra, this returns
        the named algebra as a concrete basis of operators. Conventions are
        fixed and documented in :mod:`paulie.application.algebra_basis`.

        Returns:
            list[np.ndarray]: One array per direct summand, each of shape
            ``(dim, N, N)``. See
