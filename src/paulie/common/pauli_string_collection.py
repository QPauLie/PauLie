...
class PauliStringCollection:
    ...
    def get_algebra_basis(self) -> np.ndarray:
        """
        Get the matrix basis of the dynamical Lie algebra generated by 
        the set of Pauli strings in its defining representation.
    ...
    def list_connections(self) -> list[tuple[PauliString, PauliString, int, int]]:
        ...
        """
        return [
            (x, y, len(x.get_anti_commutants(self)), len(y.get_anti_commutants(self)))
            for x, y in combinations(self.generators, r=2)
            if not x | y
        ]
