"""
    Module to compute the average graph complexity of a specified Pauli string
    with respect to the time evolution generated by a given generator set.
"""
...
def average_graph_complexity(generators: PauliStringCollection, p: PauliString) -> float:
    r"""
    Get the average graph complexity of a Pauli string with respect to the
    ...
    .. math::
        ...
        \mathbb{E}_{U\sim \mu_G}G(U^\dag p U) = \frac{1}{|C_p|}\sum_{q\in C_p}\ell(p,q)

    where :math:`C_p` is the connected component of the commutation graph containing :math:`p`
    and :math:`\ell(p,q)` is the shortest path length between :math:`p` and :math:`q` in the
    commutation graph.
    ...
    Returns:
        float: Average graph complexity of `p` with respect to the time evolution
